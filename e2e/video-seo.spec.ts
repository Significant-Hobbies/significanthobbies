...
test.describe('Video and agent discovery', () => {
  test('renders the video library with canonical metadata and no placeholder indexing', async ({
    page,
  }) => {
    await page.goto('/videos');
    await expect(
      page.getByRole('heading', { name: 'Watch something. Then do something.' })
    ).toBeVisible();
    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
      'href',
      'https://significanthobbies.com/videos'
    );
  ...
  test('negotiates public hobby pages to Markdown with canonical provenance', async ({
    request,
  }) => {
    const response = await request.get('/hobbies/photography', {
