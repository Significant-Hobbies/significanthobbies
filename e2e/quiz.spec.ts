...
test.describe("Hobby Quiz", () => {
  test("completes quiz flow and shows results", async ({ page }) => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const options = page.locator("button").filter({ hasText: /Make something|corner|long hike|Create something|High/ });
    ...
    // Should show results
    await expect(page.getByText(/recommended hobbies|Your Hobby Personality/i)).toBeVisible({ timeout: 5000 });
  });
});
