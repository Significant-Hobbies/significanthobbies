...
Journal remains useful without an account. If account sync is enabled, it keeps a private cross-device copy with explicit conflict choices. You can export or preview-replace the complete archive whenever you choose. The existing bundle and document identities remain unchanged so entries created before the product split are preserved.
