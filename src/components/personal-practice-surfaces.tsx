...
export type JournalExperienceProps = Pick<
  SharedProps,
  | 'firstName'
  | 'today'
  | 'isMorning'
...
export type HabitsExperienceProps = Pick<
  SharedProps,
  | 'firstName'
  | 'today'
  | 'habits'
...
export function JournalExperience({ saveJournalEntry, ...props }: JournalExperienceProps) {
  return (
    <PersonalPracticeSurface
      {...props}
      surface="journal"
      habits={[]}
      habitLogs={[]}
      habitCommitmentChoices={[]}
      actions={{ ...unusedActions, saveJournalEntry }}
    />
  );
}
...
function PersonalPracticeSurface({
  firstName,
  today,
  isMorning,
  weeksRemaining,
  ...
  localMode = false,
  surface,
}: SharedProps & { surface: 'journal' | 'habits' }) {
  ...
  function toggleHabit(habitId: string) {
    ...
    startTransition(async () => {
      try {
        await actions.toggleHabitLog(habitId, today, completed);
      } catch {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSaveError('Your entry was not saved. Your words are still here—try again.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6 sm:py-8">
      {preview && (
        <PreviewBanner route={surface === 'journal' ? '/journal' : '/habits'}>
          {surface === 'journal'
            ? "One stranger's week of private writing. Nothing here is yours and nothing is saved."
            : "One stranger's habit list. Check-ins are only an in-session preview and are not saved."}
        </PreviewBanner>
      )}
      <section
                ...
                <div className="space-y-6">
                  ...
                  {journalContextChoices.length > 0 && (
                    <div className="rounded-xl border border-border/60 bg-background/35 px-4 py-3.5">
                      ...
                      <select
                        id="journal-context"
                        aria-describedby="journal-context-help"
                  ...
                  <div className="flex min-h-9 flex-wrap items-center gap-3 border-t border-border/50 pt-4">
                    ...
                    {saved && (
                      <span
                        role="status"
                        aria-live="polite"
                        className="flex animate-fade-in-up items-center gap-1.5 text-sm font-medium text-primary"
                      >
            ...
            <div className="border-t border-border/60 bg-background/25 px-4 py-4 sm:px-6">
              <div className="flex items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="flex min-h-11 flex-col items-center justify-center rounded-md px-2 py-1 text-[10px] font-medium uppercase tracking-[0.18em] text-muted-foreground transition-colors hover:text-foreground disabled:cursor-default disabled:text-foreground/70"
                >
              ...
              <div className="mt-4 flex min-h-11 gap-1 overflow-x-auto pb-1">
                {journalDateWindow.map((dayDate) => {
                  ...
                  return (
                    <button
                      ...
                      className="group flex min-h-11 min-w-11 items-end justify-center rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70"
                    >
                      <span
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="font-serif text-2xl font-medium">
                    Begin with one repeatable thing.
                  </p>
                            ...
                            <div className="mt-3">
                              ...
                              {habitLinkStatus?.habitId === habit.id && (
                                ...
                                >
                                  ...
                                  {habitLinkStatus.status === 'saved' &&
                                    'Related commitment saved.'}
                                  {habitLinkStatus.status === 'error' &&
      ...
      {surface === 'journal' && <JournalArchive records={recordsWithToday} />}
    </div>
  );
}
