...
export function VideoCard({ video, featured = false }: { video: PublishedVideo; featured?: boolean }) {
  return (
    <Link
    ...
    >
      ...
      <div className={`flex flex-col justify-between ${featured ? "p-7 sm:p-9" : "p-6"}`}>
        <div>
          <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-emerald-700">Field film · {video.shortTitle}</p>
          <h2 className={`mt-4 font-serif font-semibold leading-[1.02] tracking-[-0.035em] text-stone-900 ${featured ? "text-4xl" : "text-3xl"}`}>
            {video.title}
          </h2>
        ...
        <span className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-emerald-700">
          Watch and try it <ArrowUpRight className="h-4 w-4 transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
        </span>
      </div>
    </Link>
  );
