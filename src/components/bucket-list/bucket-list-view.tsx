  ...
  return (
    <div className="overflow-hidden rounded-[1.5rem] border border-stone-200 bg-[#fffdf8] shadow-[0_18px_55px_rgba(73,60,43,0.08)]">
      <div className="border-b border-stone-200 bg-[#f6efe3] px-5 py-5 sm:px-7">
        <p className="text-[0.65rem] font-bold uppercase tracking-[0.24em] text-emerald-700">Bucket list</p>
        <h2 className="mt-1 font-serif text-3xl font-semibold tracking-tight text-stone-900 sm:text-4xl">{draft.title}</h2>
        <p className="mt-1 text-sm text-stone-500">{draft.subtitle}</p>
      </div>

      <ol className="divide-y divide-stone-100">
        {draft.items.map((item, index) => (
          <li key={item.id} className={cn("group flex items-center gap-3 px-4 py-3.5 sm:px-6", item.completedAt && "bg-emerald-50/40")}>
            <button
              type="button"
            ...
            >
              {item.completedAt ? <Check className="h-5 w-5 stroke-[2.5]" /> : <Circle className="h-5 w-5 text-stone-300" />}
            </button>
            ...
            >
              <span className={cn("block text-sm font-medium leading-snug text-stone-800 sm:text-[0.95rem]", item.completedAt && "text-stone-500 line-through decoration-emerald-600/40")}>
                {item.text}
              </span>
              <span className="mt-1 flex flex-wrap items-center gap-2 text-[0.65rem] font-semibold uppercase tracking-wider text-stone-400">
                ...
                {item.note && <span className="normal-case tracking-normal text-stone-500">“{item.note}”</span>}
              </span>
            </button>
