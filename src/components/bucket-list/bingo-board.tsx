    ...
    >
      ...
      <header className={cn("relative flex items-end justify-between gap-4", compact ? "mb-3" : "mb-5 sm:mb-7")}>
        <div className="min-w-0">
          ...
          <h2 className={cn("font-serif font-semibold leading-[0.95] tracking-[-0.035em]", compact ? "text-2xl sm:text-4xl" : "text-3xl sm:text-5xl")}>
            {draft.title}
          </h2>
          {!compact && <p className="mt-2 max-w-xl text-xs leading-relaxed text-[#637169] sm:text-sm">{draft.subtitle}</p>}
        </div>
        <div className="shrink-0 text-right">
          <p className={cn("font-serif leading-none", compact ? "text-3xl sm:text-4xl" : "text-3xl sm:text-5xl")}>
              ...
              {item.completedAt && (
                <span className="life-bingo-stamp absolute bottom-1.5 right-1.5 z-20 flex h-[43%] w-[43%] rotate-[-8deg] items-center justify-center rounded-full border-2 border-[#0f5a3c] bg-[#f9f2e3] text-[#0f5a3c] mix-blend-multiply sm:bottom-2.5 sm:right-2.5">
                  <span className="flex h-[76%] w-[76%] flex-col items-center justify-center rounded-full border border-current">
                    ...
                    <span className="text-[0.5rem] font-black uppercase tracking-[0.1em] sm:text-[0.58rem]">Lived</span>
                  </span>
                </span>
              )}
