    ...
    <div className="space-y-2">
      <div className="flex gap-2">
        <Input
          ...
          className="bg-card/40 border-border text-sm h-11"
        />
        <Button
          type="button"
          variant="outline"
          onClick={() => addHobbies(input)}
          className="border-border h-11 shrink-0"
        >
          Add
        </Button>
      </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {hobbies.map((hobby) => (
            <Badge
              key={hobby.name}
              variant="secondary"
              className="bg-growth/15 text-growth border border-growth/30 py-1 pl-2.5 pr-1 flex items-center gap-1 text-xs"
            >
              {hobby.name}
              ...
              <button
                ...
                className="-mr-0.5 flex h-7 w-7 items-center justify-center rounded-full hover:bg-growth/20 hover:text-growth"
