...
const fixture: PublishedVideo = {
  ...
  description:
    'A practical field guide to choosing one creative hobby, lowering the stakes, and completing a first small project this week.',
  ...
  transcript: [
    {
      heading: 'Start smaller than your ambition',
      paragraphs: [
        'A useful first creative session produces evidence that you can begin, not proof that you are already excellent at the craft.',
      ],
    },
  ],
  relatedArticleSlugs: [],
...
describe('video catalog', () => {
  ...
  it('rejects incomplete records', () => {
    expect(() =>
      validatePublishedVideoCatalog([{ ...fixture, description: 'Too short' }])
    ).toThrow();
  });
  ...
  it('rejects duplicate slugs, search intents, and YouTube ids', () => {
    ...
    expect(() =>
      validatePublishedVideoCatalog([fixture, { ...fixture, slug: 'another-video' }])
    ).toThrow('Duplicate video search intent');
    expect(() =>
      validatePublishedVideoCatalog([
        fixture,
        { ...fixture, slug: 'another-video', searchIntent: 'another sufficiently specific intent' },
      ])
    ).toThrow('Duplicate YouTube id');
  });
      ...
      validatePublishedVideoCatalog([
        {
          ...fixture,
          chapters: [
            { title: 'Late opening chapter', startSeconds: 20 },
            ...fixture.chapters.slice(1),
          ],
        },
      ])
    ).toThrow('must start');
      ...
      validatePublishedVideoCatalog([
        {
          ...
          chapters: [
            fixture.chapters[0],
            { title: 'Later section', startSeconds: 120 },
            { title: 'Earlier section', startSeconds: 90 },
          ],
        },
      ])
    ).toThrow('ascending');
  ...
  it('keeps schema, chapters, and YouTube publishing metadata aligned', () => {
    ...
    expect(jsonLd.hasPart[1]).toMatchObject({
      name: 'Choose a tiny first project',
      startOffset: 95,
    });
    expect(publishing.canonicalUrl).toBe(
      'https://significanthobbies.com/videos/how-to-start-a-creative-hobby'
    );
