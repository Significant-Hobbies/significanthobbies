...
export function validatePublishedVideoCatalog(input: unknown[]): PublishedVideo[] {
  ...
  for (const video of videos) {
    ...
    if (intents.has(normalizedIntent)) throw new Error(`Duplicate video search intent: ${video.searchIntent}`);
    if (youtubeIds.has(video.youtubeId)) throw new Error(`Duplicate YouTube id: ${video.youtubeId}`);
...
export function getVideoJsonLd(video: PublishedVideo) {
  ...
  return {
    ...
    hasPart: video.chapters.map((chapter, index) => ({
      ...
      ...(video.chapters[index + 1]
        ? { endOffset: video.chapters[index + 1]!.startSeconds }
        : {}),
