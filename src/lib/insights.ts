...
export function getAddedPerPhase(phases: Phase[]): string[][] {
  return phases.map((phase, i) => {
    ...
    return phase.hobbies.map((h) => h.name.toLowerCase()).filter((name) => !prev.has(name));
  });
}
...
export function getDroppedPerPhase(phases: Phase[]): string[][] {
  return phases.map((phase, i) => {
    ...
    return phases[i - 1]!.hobbies.map((h) => h.name.toLowerCase()).filter(
      (name) => !curr.has(name)
    );
  });
}
...
export function getRekindledHobbies(phases: Phase[]): string[] {
  const allHobbies = new Set(phases.flatMap((p) => p.hobbies.map((h) => h.name.toLowerCase())));
