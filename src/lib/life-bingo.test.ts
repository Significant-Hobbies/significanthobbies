...
describe("bingo progress", () => {
  it("detects horizontal, vertical, and diagonal lines", () => {
    ...
    expect(getBingoProgress({ items, size: 3 })).toMatchObject({ completed: 7, total: 9, percentage: 78 });
  });
...
describe("createRemixDraft", () => {
  it("keeps prompts while clearing completion and reflections", () => {
    ...
    expect(remix.items.map((square) => square.text)).toEqual(source.items.map((square) => square.text));
