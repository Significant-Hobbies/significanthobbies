    ...
    .map((block) => {
      switch (block.type) {
        ...
        case 'divider':
          return '---';
        case 'video':
          return `[${block.caption ?? 'Watch the video'}](${block.url})`;
        default:
          return '';
      }
    })
...
export function renderAgentContent(pathname: string): string | null {
  ...
  if (normalized === '/') {
    return document({
      ...
      description:
        'Discover meaningful hobbies, try small Side Quests, map your hobby journey, and turn future experiences into a playable Life Bingo bucket list.',
      canonical: BASE_URL,
...
- [Watch practical hobby field films](${BASE_URL}/videos)`,
  ...
  if (normalized === '/hobbies') {
    const categories = HOBBY_CATEGORIES.map(
      (category) =>
        `## ${category.name}\n\n${category.hobbies.map((hobby) => `- [${hobby}](${BASE_URL}/hobbies/${hobbySlug(hobby)})`).join('\n')}`
    ).join('\n\n');
    return document({
      title: 'Hobby directory',
      description: 'The Significant Hobbies taxonomy, grouped into ten practical categories.',
      canonical: `${BASE_URL}/hobbies`,
      body: `# Hobby directory\n\n${categories}`,
    });
  }
...
[Add a ${hobby} experience to your Bucket List](${BASE_URL}/bucket-list/new?idea=${encodeURIComponent(`Try ${hobby}`)})`,
  ...
  if (normalized === '/blog') {
    return document({
      ...
      description: 'Articles about hobbies, identity, curiosity, and making life less repetitive.',
      canonical: `${BASE_URL}/blog`,
      body: `# The Hobby Journal\n\n${blogPosts.map((post) => `- [${post.title}](${BASE_URL}/blog/${post.slug}) — ${post.excerpt}`).join('\n')}`,
    });
  }
    ...
    const list =
      publishedVideos.length > 0
        ? publishedVideos
            .map(
              (video) =>
                `- [${video.title}](${BASE_URL}/videos/${video.slug}) — ${video.description}`
            )
            .join('\n')
        : 'The first Significant Hobbies field films are in production. No placeholder videos are indexed.';
    return document({
      title: 'Significant Hobbies field films',
      description:
        'Practical hobby videos with written takeaways, chapters, and one thing to try next.',
      canonical: `${BASE_URL}/videos`,
      body: `# Watch something. Then do something.\n\n${list}`,
    });
  }
...
- [Add this experience to a Bucket List](${BASE_URL}/bucket-list/new?idea=${encodeURIComponent(video.bucketListPrompt)})`,
  ...
  if (normalized === '/life-bingo') {
    return document({
      title: 'Life Bingo',
      description:
        'A playable Bucket List that turns a life chapter into nine or twenty-five concrete experiences.',
      canonical: `${BASE_URL}/life-bingo`,
  ...
  if (normalized === '/side-quests') {
    return document({
      title: 'Side Quests',
      description: 'Fifty small, concrete adventures that make an ordinary week more memorable.',
      canonical: `${BASE_URL}/side-quests`,
...
Search and reference use are allowed. Model training is not allowed. Canonical HTML pages remain the source of truth.`;
  ...
  const hobbies = HOBBY_CATEGORIES.map(
    (category) =>
      `### ${category.name}\n${category.hobbies.map((hobby) => `- [${hobby}](${BASE_URL}/hobbies/${hobbySlug(hobby)})`).join('\n')}`
  ).join('\n\n');
  ...
  const videos =
    publishedVideos
      .map((video) => `- [${video.title}](${BASE_URL}/videos/${video.slug}): ${video.description}`)
