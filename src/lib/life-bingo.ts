...
export const BINGO_INTENTIONS = [
  { id: 'adventure', label: 'Adventure', emoji: '↗', description: 'Novel places and small leaps' },
  {
    ...
    label: 'Creativity',
    emoji: '✦',
    description: 'Make things without needing to be good',
  },
  {
    id: 'connection',
    label: 'Connection',
    emoji: '∞',
    description: 'More stories with other people',
  },
  { id: 'courage', label: 'Courage', emoji: '!', description: 'A life a little outside the usual' },
  {
    ...
    label: 'Curiosity',
    emoji: '?',
    description: 'Follow ideas and learn by doing',
  },
...
const SUGGESTIONS: Record<BingoIntention, Suggestion[]> = {
  adventure: [
    {
      text: 'Take the train somewhere you have never stopped',
      intention: 'adventure',
      effort: 'medium',
    },
    {
      text: 'Spend a day exploring without an itinerary',
      intention: 'adventure',
      effort: 'medium',
    },
    { text: 'Eat somewhere you cannot pronounce the menu', intention: 'adventure', effort: 'tiny' },
    {
      text: 'Book one night somewhere close but unfamiliar',
      intention: 'adventure',
      effort: 'bold',
    },
    {
      text: 'Say yes to an invitation you would usually skip',
      intention: 'adventure',
      effort: 'medium',
    },
    {
      text: 'Watch sunrise from a place you have never been',
      intention: 'adventure',
      effort: 'medium',
    },
  ...
  creativity: [
    ...
    {
      text: 'Take a pottery, printmaking, or sewing class',
  ...
  connection: [
    {
      text: 'Invite someone over without making it fancy',
    ...
    {
      text: 'Host a table where two guests have never met',
      intention: 'connection',
      effort: 'bold',
    },
    {
      text: 'Make a new friend through a shared interest',
      intention: 'connection',
      effort: 'bold',
    },
    {
      text: 'Plan the reunion instead of saying ‘we should’',
      intention: 'connection',
      effort: 'medium',
    },
    { text: 'Learn and cook someone’s family recipe', intention: 'connection', effort: 'medium' },
    {
      text: 'Give someone your full attention for an hour',
      intention: 'connection',
      effort: 'tiny',
    },
  ],
  courage: [
    ...
    {
      text: 'Ask for something you normally stay quiet about',
      intention: 'courage',
      effort: 'medium',
    },
    ...
    {
      text: 'Do one thing alone that you usually need company for',
      intention: 'courage',
      effort: 'medium',
    },
  ...
  learning: [
    {
      text: 'Attend a talk about something completely new',
      intention: 'learning',
      effort: 'medium',
    },
    ...
    {
      text: 'Visit a museum and follow one question home',
      intention: 'learning',
      effort: 'medium',
    },
    { text: 'Ask an expert to show you how they work', intention: 'learning', effort: 'bold' },
    {
      text: 'Learn to identify five birds, trees, or stars',
      intention: 'learning',
      effort: 'tiny',
    },
    {
      text: 'Finish a small course with something you made',
      intention: 'learning',
      effort: 'bold',
    },
    {
      text: 'Spend an hour following a ridiculous curiosity',
      intention: 'learning',
      effort: 'tiny',
    },
  ],
  ...
  wellbeing: [
    ...
    {
      text: 'Make a meal slowly with no other task running',
      intention: 'wellbeing',
      effort: 'medium',
    },
    { text: 'Take yourself somewhere quiet for an hour', intention: 'wellbeing', effort: 'tiny' },
    {
      text: 'Create a bedtime ritual you look forward to',
      intention: 'wellbeing',
      effort: 'medium',
    },
...
const HORIZON_COPY: Record<
  BingoHorizon,
  { title: string; subtitle: string; size: BingoBoardSize }
> = {
  month: {
    title: 'A month worth remembering',
    subtitle: 'Nine small invitations to make the ordinary feel different.',
    size: 3,
  },
  season: {
    title: 'My season of possibility',
    subtitle: 'A little more life before the season changes.',
    size: 3,
  },
  year: {
    title: `My ${new Date().getFullYear()} Life Bingo`,
    subtitle: 'Twenty-five chances to make this year feel like mine.',
    size: 5,
  },
  chapter: {
    title: 'The next chapter',
    subtitle: 'Things I want to experience, not just accomplish.',
    size: 5,
  },
};
...
export function generateLifeBingo(input: {
  ...
  const desiredEfforts: BingoEffort[] =
    input.boldness === 'cozy'
      ? ['tiny', 'tiny', 'medium']
      : input.boldness === 'bold'
  ...
  const secondaryCandidates = BINGO_INTENTIONS.map((intention) => intention.id)
    ...
    const match =
      candidates.find(
        (item) =>
          ...
          item.effort === effort &&
          !chosen.some((picked) => picked.text === item.text)
      ) ??
      candidates.find(
        (item) =>
          item.intention === intention && !chosen.some((picked) => picked.text === item.text)
      ) ??
...
export function replacementSquare(input: {
  ...
  const intention =
    current.intention === 'wildcard' ? input.draft.intentions[0]! : current.intention;
  const random = seededRandom(input.seed ?? `${Date.now()}-${input.squareId}`);
  const available = shuffled(SUGGESTIONS[intention], random).filter(
    (item) => !existingText.has(item.text)
  );
  const suggestion =
    available.find((item) => effortAllowed(item.effort, input.draft.boldness)) ?? available[0];
...
export function getBingoLines(items: BucketListItem[], size: BingoBoardSize): BingoLine[] {
  const positionedItems = items.some((item) => typeof item.boardPosition === 'number')
    ? items
        .filter(
          (item) => typeof item.boardPosition === 'number' && item.boardPosition < size * size
        )
  ...
  const complete = (indexes: number[]) =>
    indexes.every((index) => Boolean(positionedItems[index]?.completedAt));
...
export function isBucketListDraft(value: unknown): value is BucketListDraft {
  ...
  return (
    draft.version === LIFE_BINGO_DRAFT_VERSION &&
