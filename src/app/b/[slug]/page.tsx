...
function getDemoDraft() {
  ...
  draft.items = draft.items.map((item, index) =>
    [0, 4].includes(index) ? { ...item, completedAt: "2026-07-12T00:00:00.000Z", note: "A day worth keeping." } : item,
  );
  return draft;
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    robots: result.list.visibility === "PUBLIC" ? { index: true, follow: true } : { index: false, follow: false },
    openGraph: {
      title: result.list.title,
      description: result.list.subtitle || "A bucket list in its most playable form.",
      type: "website",
