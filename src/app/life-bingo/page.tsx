...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    images: [
      {
        url: '/life-bingo/opengraph-image',
        width: 1200,
        height: 630,
        alt: 'Create a playable Life Bingo bucket list',
      },
    ],
  },
  twitter: {
    ...
    title: 'Life Bingo — Make Life Less Repetitive',
    description: 'A bucket list you can actually play.',
    images: ['/life-bingo/opengraph-image'],
  },
};
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-14 lg:grid-cols-[0.88fr_1.12fr] lg:gap-20">
          <div>
            ...
            <p className="mt-6 max-w-xl text-base leading-relaxed text-stone-600 sm:mt-7 sm:text-xl">
              Turn the things you keep saying “someday” about into a beautiful board of real
              experiences. Then go live them.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button
                asChild
                size="lg"
                className="h-12 rounded-xl bg-[#176b4a] px-6 text-white shadow-[0_10px_30px_rgba(23,107,74,0.2)] hover:bg-[#10583d]"
              >
                <Link href="/bucket-list/new">
                  Make my Life Bingo <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
              <span className="flex items-center justify-center gap-2 px-3 text-xs font-medium text-stone-500 sm:justify-start">
                <Check className="h-3.5 w-3.5 text-emerald-700" /> No account needed
              </span>
            </div>
          </div>
          ...
          <div className="grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-24">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.28em] text-emerald-700">
                Not another goal tracker
              </p>
              <h2 className="mt-4 font-serif text-5xl font-semibold leading-[0.95] tracking-[-0.04em] sm:text-6xl">
      ...
      <section className="px-4 py-20 sm:py-28">
        <div className="mx-auto max-w-5xl rounded-[2rem] bg-[#1e3029] px-6 py-12 text-[#fffaf0] shadow-[0_30px_90px_rgba(30,48,41,0.24)] sm:px-12 sm:py-16">
          <div className="grid items-end gap-10 md:grid-cols-[1fr_auto]">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.26em] text-[#a8c5b5]">
                One list. Two ways to use it.
              </p>
              <h2 className="mt-4 max-w-3xl font-serif text-5xl font-semibold leading-[0.95] tracking-[-0.04em] sm:text-6xl">
                Plan it as a list. Live it as Bingo.
              </h2>
              <div className="mt-7 flex flex-wrap gap-x-6 gap-y-3 text-sm text-[#d8e2dc]">
                <span className="flex items-center gap-2">
                  <Circle className="h-3.5 w-3.5" /> Add your own ideas
                </span>
                <span className="flex items-center gap-2">
                  <Circle className="h-3.5 w-3.5" /> Mark moments lived
                </span>
                <span className="flex items-center gap-2">
                  <Circle className="h-3.5 w-3.5" /> Export the board
                </span>
              </div>
            </div>
            <Button
              asChild
              size="lg"
              className="h-12 rounded-xl bg-[#f3d776] px-6 text-[#1e3029] hover:bg-[#f7e39a]"
            >
              <Link href="/bucket-list/new">
