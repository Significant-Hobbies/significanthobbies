    ...
    <main>
      ...
      <section className="relative overflow-hidden bg-stone-950 text-white">
        <div className="pointer-events-none absolute inset-0" aria-hidden>
          {[[15,10],[80,20],[40,35],[70,8],[20,60],[90,75],[55,15],[10,80],[65,50]].map(([x,y],i)=>(
            <span key={i} className="absolute rounded-full bg-white opacity-20"
              style={{left:`${x}%`,top:`${y}%`,width:i%3===0?"2px":"1px",height:i%3===0?"2px":"1px"}}/>
          ))}
        </div>
        <div className="relative mx-auto max-w-4xl px-4 py-20 text-center space-y-6">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold leading-tight">
            Bucket list ideas<br />
            <span className="text-amber-400">worth doing before you die</span>
          </h1>
          ...
          return (
            <section key={key} id={key} className="scroll-mt-28 space-y-6">
              <div className="flex items-center gap-3">
                <div className={`h-10 w-10 rounded-xl border ${style.border} ${style.bg} flex items-center justify-center text-xl`}>
                  {cat.emoji}
                </div>
                <div>
                  <h2 className={`text-2xl font-bold text-stone-900`}>{cat.label}</h2>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {cat.ideas.map((idea, i) => (
                  <div key={i} className={`flex items-start gap-3 rounded-xl border ${style.border} ${style.bg} px-4 py-3 group`}>
              ...
              {(() => {
                ...
                return (
                  <div className={`rounded-xl border ${style.border} ${style.bg} px-5 py-4`}>
                    <p className={`text-xs font-semibold uppercase tracking-wider ${style.text} mb-3`}>
                      Famous people who checked {cat.label.toLowerCase()} off their list
                    </p>
                    <div className="flex flex-wrap gap-3">
                      {famous.map(p => (
                        <Link key={p.slug} href={`/bucket-lists/${p.slug}`}
      ...
      <section className="bg-amber-50 border-t border-amber-100">
        <div className="mx-auto max-w-3xl px-4 py-16 text-center space-y-6">
          ...
          <p className="text-stone-600 max-w-md mx-auto">
            Lumi tracks your bucket list, shows your personality archetype, and matches
            you to the famous person whose ambitions look most like yours.
          </p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/dashboard"
